...
context "Get Advisory Lock" do
  ...
  comment "Pool Size: #{advisory_lock_pool_size.inspect}"

  stream_id = Controls::ID::Random.example
  stream_id_hash64 = Hash64.get_unsigned(stream_id)
  stream_id_partition = stream_id_hash64 % advisory_lock_pool_size
  ...
  control_advisory_lock_hex = "#{category_hash64.to_s(16).rjust(6, '0')}#{stream_id_partition.to_s(16).rjust(2, '0')}"
  ...
  detail "  Stream ID Partition: #{stream_id_partition}, 0x#{stream_id_partition.to_s(16).rjust(2, '0')} (Hash64: 0x#{stream_id_hash64.to_s(16)})"
